//
//      This example shows how to get list of functions start-end.
//
...
static ExportFunctions(filename,total){
  ...
  {  
      ...
      if(end_addr!=-1)
      {
        ...
        {
          total++;
          str=form("%08lX:%08lX",ea,end_addr);
          str=str+"\n";        
          fprintf(file,str);
...
static main() {
  ...
  Message("\n\nExporting Functions...\n");
  ExportFunctions("C:\\pvmap.map",GetNumFunctions());

  Message("\nExporting Data Blocks...\n");
  ExportData("C:\\pvmap.map");

  Message("\n\nFile 'c:\\pvmap.map' Created.");
}
